...
*   **Clerk Auth**
    ...
    *   Clerk Auth 负责用户认证管理，为用户提供安全、简单且多方式兼容的登录体验。它确保只有经过身份验证的用户才能访问系统。
...
*   **安全措施**
    ...
    *   使用 Clerk Auth 来确保用户认证过程的安全，只有经过验证的用户才能使用应用。
...
*   **前端**：Next.js、Tailwind CSS、TypeScript、Shadcn UI、Framer Motion
*   **后端**：Tauri 2.0、Rust、Supabase 数据库、Clerk Auth
