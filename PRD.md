...
#### 2.1.1 AI对话界面
- **对话输入区**：用户可以输入自然语言指令
- **对话历史**：显示与AI的完整对话历史，包括工具执行结果
- **实时响应**：AI回复和工具执行结果的实时显示
- **会话管理**：创建、保存和加载会话
- **系统提示词配置**：允许自定义系统提示词以优化AI行为
- **API提供商选择**：支持Anthropic API、AWS Bedrock和Google Vertex AI

#### 2.1.2 计算机控制工具 (ComputerTool)
- **屏幕截图**：捕获当前屏幕内容并发送给AI分析
- **鼠标控制**：
  - 移动鼠标到指定坐标位置
  - 单击、双击、右键点击、中键点击
  - 三击操作（20250124版API支持）
  - 鼠标按下和释放分离操作（20250124版API支持）
  - 拖拽操作
  - 滚动操作（上、下、左、右方向，可指定滚动量，20250124版API支持）
- **键盘控制**：
  - 输入文本（支持分组输入和延迟模拟真实打字）
  - 按键操作（单个按键和组合键）
  - 按键保持和释放（20250124版API支持）
- **等待和延时**：设置操作间的等待时间（20250124版API支持）
- **光标位置获取**：获取当前鼠标位置
- **分辨率适配**：自动缩放坐标和截图以适应不同分辨率，支持最大分辨率限制

#### 2.1.3 命令执行工具 (BashTool)
- **命令行执行**：在持久的bash会话中运行系统命令
- **命令输出捕获**：实时捕获和显示命令执行的标准输出和错误输出
- **会话管理**：维护持久的bash会话，支持会话重启
- **超时处理**：命令执行超时保护机制（默认120秒）
- **错误处理**：优雅处理命令执行错误和异常情况

#### 2.1.4 文本编辑工具 (EditTool)
- **文件查看**：读取文件内容，支持指定行范围查看
- **目录浏览**：查看目录内容（最多2层深度）
- **文件创建**：创建新文件并写入内容
- **文本替换**：在文件中查找并替换指定文本（仅支持唯一匹配）
- **文本插入**：在指定行插入新文本
- **编辑撤销**：撤销最近的文件编辑操作
- **文件历史**：维护文件编辑历史，支持版本回退
- **路径验证**：安全验证文件路径，防止越权访问
- **结果格式化**：以行号格式显示文件内容

#### 2.1.5 工具集成框架
- **工具版本管理**：支持不同版本的API工具集（20241022和20250124）
- **工具组合**：灵活组合不同工具以满足特定需求
- **异步执行**：所有工具操作采用异步执行模式
- **结果格式化**：统一的工具结果格式，支持文本和图像输出
- **错误处理**：标准化的错误处理机制
- **工具集合**：通过ToolCollection管理多个工具的组合
...
| 0.5  | 更新 | 更新技术栈为Vite、React、Tailwind CSS、shadcn/ui、Lucide React、Framer Motion和Tauri v2 | Claude |
